...
void handleDirectMethod(String topicStr, String payloadStr) 
{
    ...
    Serial.println("[IoT] Received Command");
    Serial.println("      Command Name:  " + methodName);
    Serial.println("      Command Value: " + payloadStr);
    Serial.println("[IoT] Checking command name");

    if (strcmp(methodName.c_str(), "CHANGEDOORSTATE") == 0) // check if the command is to change the door state
    {
        Serial.println("[IoT] Change Door State Command Received");
        Serial.println("[IoT] Changing Door State");
    ...
    {
        Serial.println("[IoT] Command not recognised, ignoring");
    }
}
...
void handleTwinPropertyChange(String topicStr, String payloadStr) 
{
    ...
    char *arr[] = {"lockMode", "showneo", "telemetrySendInterval", "unlockPIN"};
...
void callback(char* topic, byte* payload, unsigned int length) 
{
    ...
    if (topicStr.startsWith(F("$IOTHUB/METHODS/POST/"))) { // direct method callback
        ...
        if (msgId == twinRequestId) 
        {
            ...
            char* keys[] = {"telemetrySendInterval", "unlockPIN", "lockMode", "showneo"};
