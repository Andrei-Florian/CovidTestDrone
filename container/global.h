...
public:
    ...
    void getData()
    {
        ...
        this->telemetry.containerDoorState = ldrSensor.check();
...
public:
    ...
    void sendTelemetry()
    {
        ...
        payload.replace(F("{containerDoorState}"), String(global.telemetry.containerDoorState));
    ...
    void sendDoorState()
    {
        ...
        payload.replace(F("{LOCATION}"), "\"actionLocation\": { \"lat\": {lat}, \"lon\": {lon}, \"alt\": {alt}}");
        payload.replace(F("{lat}"), String(global.location.latitude, 7));
        payload.replace(F("{lon}"), String(global.location.longitude, 7));
        payload.replace(F("{alt}"), String(global.location.altitude, 7));
